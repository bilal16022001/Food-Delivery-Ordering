    ...
    include "SideBar.php";
     ?>
      <div class="users profile">
          ...
          <ul>
              <?php
                 $admin = getdb("*","users","WHERE","Role = 1");
     ...
     if($_SERVER['REQUEST_METHOD'] == "POST"){
            ...
            $stmt = $con->prepare("UPDATE `users` SET `FirstName` = '$firstName' , `LastName` = '$lastName' , `Phone` = '$phone' , `password` = '$sqlPas' , `Email` = '$email' WHERE `users`.`Role` = 1 ");
            $stmt->execute();
     }
